...
# Ensure this checkout takes precedence over an installed mmdet3d package when
# the entry script is launched as ``python tools/train_geometry_only.py``.
MMDET_ROOT = osp.dirname(osp.dirname(osp.abspath(__file__)))
...
def parse_args():
    ...
    parser.add_argument(
        'config',
        nargs='?',
        default='configs/minkunet/minkunet34_w32_minkowski_geometry_8xb2-laser-polar-mix-3x_semantickitti.py',
        help='geometry-only train config file path')
    parser.add_argument(
        '--pretrained',
        default='ckpt/minkunet34_w32_minkowski_8xb2-laser-polar-mix-3x_semantickitti_20230514_202236-839847a8.pth',
