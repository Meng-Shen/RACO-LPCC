...
def parse_scale_value(text):
    """Parse decimal or fractional quantization scales such as ``1/64``."""
    text = str(text).strip()
    if not text:
...
def parse_quant_map(text):
    """Parse ``fg,bg;fg,bg`` quantization combinations in their given order."""
    combinations = []
    for item in str(text).split(";"):
        ...
        if len(parts) != 2:
            raise ValueError(
                f"Invalid quant map item: {item}. Expected format: fg,bg"
            )
        combinations.append(
            (parse_scale_value(parts[0]), parse_scale_value(parts[1]))
        )
...
def main():
    ...
    parser.add_argument("--manifest", required=True, help="Router-label manifest containing label_csvs")
