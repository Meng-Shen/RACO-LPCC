...
EVAL_SCRIPT="${ROOT_DIR}/scripts/evaluation/evaluate_kitti_uniform_quantization.py"
PARSE_SCRIPT="${ROOT_DIR}/scripts/curve_tools/parse_kitti_detection_baseline_ap_log.py"
...
CFG_FILE="${CFG_FILE:-${ROOT_DIR}/integrations/openpcdet/configs/kitti_models/pv_rcnn_fov_geometry.yaml}"
CKPT="${CKPT:-ckpt/model_non_reflectance.pth}"
BATCH_SIZE="${BATCH_SIZE:-8}"
WORKERS="${WORKERS:-4}"
# Keep rate IDs aligned with router labels: 0 is coarsest, 5 is finest.
SCALES="${SCALES:-1/2048,1/1024,1/512,1/256,1/128,1/64}"
...
GPCC_CFG="${GPCC_CFG:-${ROOT_DIR}/extension/kitti.cfg}"

OUT_DIR="${OUT_DIR:-${ROOT_DIR}/experiment_results/baseline_fov}"
...
require_file "$PARSE_SCRIPT"

if [[ ! -d "$KITTI_VELODYNE" ||
      ! -f "${KITTI_ROOT}/fov_crop_stats.csv" ]]; then
  log "FOV-only KITTI data is missing; generating it first"
  OUTPUT_ROOT="$KITTI_ROOT" \
    "$ROOT_DIR/scripts/data_preparation/prepare_kitti_fov.sh"
fi
...
if [[ "$RUN_AP" == "1" ]]; then
  ...
  find "${ROOT_DIR}/OpenPCDet/output" -name 'log_eval_pos_*.txt' 2>/dev/null | sort > "$before_file" || true
  "$PYTHON_BIN" "$EVAL_SCRIPT" \
    ...
    --workers "$WORKERS" \
    --scales "$SCALES"
  find "${ROOT_DIR}/OpenPCDet/output" -name 'log_eval_pos_*.txt' 2>/dev/null | sort > "$after_file" || true
  AP_LOG="$(comm -13 "$before_file" "$after_file" | tail -n 1)"
  if [[ -z "$AP_LOG" ]]; then
    AP_LOG="$(find "${ROOT_DIR}/OpenPCDet/output" -name 'log_eval_pos_*.txt' 2>/dev/null | sort | tail -n 1)"
  fi
  require_file "$AP_LOG"
  log "Parsing AP log: $AP_LOG"
  "$PYTHON_BIN" "$PARSE_SCRIPT" --log "$AP_LOG" --out "$AP_CSV"
else
...
cd "$ROOT_DIR"
"$PYTHON_BIN" \
  scripts/curve_tools/merge_kitti_detection_gpcc_baseline_curve.py \
  --ap_csv "$AP_CSV" \
