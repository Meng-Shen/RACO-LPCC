#!/usr/bin/env python3
"""Export decoded-domain SemanticKITTI mIoU loss at six geometry steps.

For every quantized cloud, predictions are evaluated directly on decoded
points after nearest-neighbour ground-truth recolouring from the original
cloud.  The finest available operating point (64 mm) is the zero-loss
reference, and the router target is decoded_mIoU(64 mm) - decoded_mIoU(q).
Original-point nearest-neighbour evaluation is retained only for auditing.
Eight independent torchrun workers write resumable shards; ``--merge-only``
validates and combines them.
"""
...
def parse_args():
    ...
    parser.add_argument('--loss-thresholds', type=float, nargs=6,
                        default=(0.05, 0.15, 0.25, 0.35, 0.45, 0.55))
...
@torch.no_grad()
def predict_labels(model, xyz, device):
    from mmdet3d.structures import Det3DDataSample, PointData

    points = torch.from_numpy(xyz).to(device=device, dtype=torch.float32)
    ...
    if len(prediction) != len(xyz):
        raise RuntimeError(
            f'Prediction/point mismatch: {len(prediction)} vs {len(xyz)}')
    return prediction
...
def fieldnames(steps_mm):
    fields = [
        'frame_id',
        'baseline_label',
        'baseline_quant_step_mm',
        'baseline_decoded_miou',
        'original_points',
    ]
    for label, _ in enumerate(steps_mm):
        fields.extend([
            ...
            f'L{label}_point_error_delta',
            f'L{label}_loss_delta',
            f'L{label}_miou',
            f'L{label}_miou_loss',
            f'L{label}_valid_points',
            f'L{label}_quantized_points',
            f'L{label}_decoded_point_error',
            f'L{label}_decoded_miou',
            f'L{label}_decoded_valid_points',
        ])
    return fields
    ...
    with shard_path.open('a', newline='', buffering=1) as handle:
        ...
        for local_index, item in enumerate(pending, 1):
            ...
            for step_mm in args.steps_mm:
                ...
                quantized_prediction = predict_labels(
                ...
                decoded_ground_truth = transfer_with_nearest_neighbor(
                    quantized_xyz, original_xyz, ground_truth)
                decoded_error, decoded_miou, decoded_valid = \
                ...
                step_stats.append({
                    ...
                    'decoded_point_error': decoded_error,
                    'decoded_miou': decoded_miou,
                    'decoded_valid_points': decoded_valid,
                })

            baseline_label = len(args.steps_mm) - 1
            baseline_stats = step_stats[baseline_label]
            row = {
                'frame_id': item['frame_id'],
                'baseline_label': baseline_label,
                'baseline_quant_step_mm': args.steps_mm[baseline_label],
                'baseline_decoded_miou':
                f'{baseline_stats["decoded_miou"]:.10f}',
                'original_points': len(original_xyz),
            }
            for label, (step_mm, stats) in enumerate(
                ...
                row.update({
                    ...
                    f'L{label}_point_error_delta':
                    f'{stats["point_error"] - baseline_stats["point_error"]:.10f}',
                    f'L{label}_loss_delta':
                    f'{baseline_stats["decoded_miou"] - stats["decoded_miou"]:.10f}',
                    ...
                    f'L{label}_decoded_point_error':
                    f'{stats["decoded_point_error"]:.10f}',
                    f'L{label}_decoded_miou':
                    f'{stats["decoded_miou"]:.10f}',
                    f'L{label}_decoded_valid_points':
                    stats['decoded_valid_points'],
                })
            writer.writerow(row)
            if local_index % 10 == 0:
                handle.flush()
...
def merge_shards(args, items):
    ...
    for rank in range(args.expected_world_size):
        ...
        with path.open(newline='') as handle:
            ...
            for row in reader:
                ...
                if not 0.0 <= baseline_miou <= 1.0:
                    raise ValueError(
                        f'Invalid baseline mIoU for {frame_id}')
                if abs(float(row['L5_loss_delta'])) > 1e-8:
                    raise ValueError(
                        f'L5 is not the zero-loss baseline for {frame_id}')
    ...
    for rate_id, threshold in enumerate(args.loss_thresholds):
        ...
        with path.open('w', newline='') as handle:
            fields = ['frame_id', 'jucp_label', 'rate_id', 'threshold',
                      'quant_step_mm', 'baseline_decoded_miou',
                      'selected_decoded_miou', 'selected_miou_drop',
                      'selected_original_domain_miou']
            ...
            for frame_id in expected_ids:
                ...
                writer.writerow({
                    ...
                    'baseline_decoded_miou':
                    row['baseline_decoded_miou'],
                    'selected_decoded_miou':
                    row[f'L{chosen}_decoded_miou'],
                    'selected_miou_drop': row[f'L{chosen}_loss_delta'],
                    'selected_original_domain_miou':
                    row[f'L{chosen}_miou'],
                })
    ...
    manifest = {
        'mode': 'semantic_kitti_xyz19_decoded_miou_drop_from_64mm',
        ...
        'reference_definition': (
            'per-frame decoded-domain mIoU at the finest 64mm step'),
        'primary_metric': (
            'per-frame mIoU on decoded points after nearest-neighbour '
            'ground-truth recolouring from the original cloud'),
        'audit_metric': (
            'per-frame mIoU after one-way nearest-neighbour prediction '
            'transfer from decoded points to original non-ignore points'),
        'loss_delta_definition': (
            'decoded_miou_at_64mm - candidate_decoded_domain_miou'),
