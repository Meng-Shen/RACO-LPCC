#!/usr/bin/env python3
"""Extract per-object ShapeNet55 CE loss and predictions at quantization levels."""
...
def load_model(source_dir: Path, checkpoint_path: Path, device):
...
def evaluate(model, points, labels, indices, qstep, batch_size, workers, device, classes):
    ...
    with torch.no_grad():
        for xyz, target, source_index, unique_count in loader:
            ...
            logits = model(xyz)
            losses.append(F.cross_entropy(logits, target_gpu, reduction="none").cpu().numpy())
...
def main():
    ...
    model, classes = load_model(Path(args.source_dir), Path(args.checkpoint), device)
