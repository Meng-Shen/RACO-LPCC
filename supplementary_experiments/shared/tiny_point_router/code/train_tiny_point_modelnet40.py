#!/usr/bin/env python3
"""Train TinyPoint six-absolute-loss + monotonic-BPP router on ModelNet40.
...
def collate(batch):
    ...
    if len(point_counts) != 1:
        raise ValueError("ModelNet40 TinyPoint training expects fixed-size point sets")
    return {
...
@torch.no_grad()
def load_legacy_modelnet_heads(model: nn.Module, checkpoint_path: Path):
    """Reuse safe parts of the old five-delta-loss ModelNet router heads.
    ...
    return {
        ...
        "mode": "legacy_ModelNet_five_delta_heads_to_TinyPoint_six_absolute_heads",
...
def main() -> None:
    ...
    test_indices = np.arange(len(np.load(args.test_points, mmap_mode="r")))
    ...
    val_set = RoutingObjects(args.train_points, args.train_quant, args.train_bpp, val_indices)
    ...
    val_loader = make_loader(val_set, batch_size, workers, False)
    ...
    init_report = load_legacy_modelnet_heads(model, args.init_checkpoint)
    ...
    run_info = {
        **vars(args),
        ...
        "loss_target": "quantization-level absolute DGCNN cross entropy (NPZ losses)",
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        save_checkpoint(
            output / "latest.pth", model, optimizer, scheduler, epoch, val_metrics,
        ...
        if score > best_score + 1e-8:
            ...
            save_checkpoint(
                output / "best.pth", model, optimizer, scheduler, epoch, val_metrics,
                args, lambdas, loss_scales, mean_log_bpp, train_set.qsteps, init_report,
            )
        else:
            stale += 1
    ...
    summary = {
        ...
        "model_type": "TinyPoint backbone + six independent absolute-loss heads + monotonic six-BPP head",
        "optimization_targets": "six absolute DGCNN losses + true G-PCC BPP only",
