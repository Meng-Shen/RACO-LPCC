...
[[ -s "$CHECKPOINTS/centerpoint_voxel01_circlenms_nuscenes_official.pth" ]]
[[ -s "$CHECKPOINTS/kitti_rate_aware_5loss_plus_bpp_best.pth" ]]
[[ -s "$TRAIN_BPP" && -s "$VAL_BPP" ]]
"$PYTHON" -c "from mmengine.config import Config; c=Config.fromfile('$CONFIG'); assert c.train_cfg.max_epochs == 12; assert c.train_cfg.val_interval == 1; h=c.default_hooks.checkpoint; assert h.save_best == 'NuScenes metric/pred_instances_3d_NuScenes/mAP'; assert h.rule == 'greater'; assert c.work_dir == '$DETECTOR_OUT'; print('preflight: overall mAP save_best, 12 epochs, validation every epoch')"

record "Stage 2/7: train XYZ-only CenterPoint for 12 epochs on GPU 0"
if [[ ! -f "$DETECTOR_OUT/DETECTOR_COMPLETE" ]]; then
    ...
    DETECTOR_METRIC=$("$PYTHON" -c "import json; x=json.load(open('$DETECTOR_SELECTION')); print(f\"epoch={x['best_epoch']} overall_mAP={x['best_mAP']:.6f}\")")
    [[ -s "$DETECTOR_CKPT" ]]
...
record "ALL DONE: 12-epoch detector selected by official overall mAP; labels, proxy, and mAP-BPP rebuilt"
