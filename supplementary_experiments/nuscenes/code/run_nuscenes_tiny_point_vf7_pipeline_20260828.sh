...
WARM_INIT="$BASE/experiments/nuscenes_tiny_point_vf7_20260828/checkpoints/epoch_008.pth"
...
TRAIN_OUT="$BASE/experiments/nuscenes_tiny_point_vf7_alltrain_trainloss_20260828"
SMOKE_OUT="$BASE/experiments/nuscenes_tiny_point_vf7_alltrain_trainloss_smoke_20260828"
PLOT_OUT="$BASE/experiments/nuscenes_tiny_point_vf7_alltrain_trainloss_vs_gpcc_20260828"
STATE="$TRAIN_OUT/state"
LOG="$BASE/nuscenes_tiny_point_vf7_alltrain_trainloss_20260828_pipeline.log"

mkdir -p "$TRAIN_OUT" "$SMOKE_OUT" "$PLOT_OUT" "$STATE"
exec 9>"$BASE/.nuscenes_tiny_point_vf7_alltrain_trainloss_20260828.lock"
if ! flock -n 9; then
  echo "The nuScenes TinyPoint-VF7 pipeline is already active."
  exit 2
fi
exec >>"$LOG" 2>&1
...
COMMON_ARGS=(
  ...
  --lambdas $LAMBDAS --model-variant tiny_point_vf7 --init-kind tiny_point_full
  --init-checkpoint "$WARM_INIT"
  --voxel-size 0.16 0.16 0.16
  --point-cloud-range -51.2 -51.2 -5.0 51.2 51.2 3.0
  --max-voxels 50000 --feat-dim 256 --workers 2
  --backbone-lr 1e-3 --head-lr 5e-4 --weight-decay 5e-4
  --loss-weight 2.0 --rate-weight 1.0 --jitter-std 0.005 --seed 20260828
)

record "Stage 1/6 preflight and smoke test"
for path in "$TRAIN_LOSS" "$VAL_LOSS" "$TRAIN_SPLIT" "$VAL_SPLIT" "$TEST_SPLIT" \
  "$TRAIN_BPP" "$VAL_BPP" "$LAMBDA_JSON" "$WARM_INIT" "$FIXED_CURVE"; do
  [[ -s "$path" ]]
done
[[ $(find "$PREDICTION_ROOT" -type f -name predictions.manifest.json | wc -l) -eq 6 ]]
$PYTHON -m py_compile \
  "$CODE/tiny_point_absolute_loss_monotonic_rate_proxy.py" \
  "$CODE/tiny_point_vf7_absolute_loss_monotonic_rate_proxy.py" \
  "$CODE/train_nuscenes_tiny_point_vf7_router_ddp.py" \
  "$CODE/export_nuscenes_tiny_point_vf7_candidates_multigpu.py" \
  "$CODE/plot_nuscenes_tiny_point_vf7_vs_gpcc.py"
if [[ ! -s "$SMOKE_OUT/TRAINING_COMPLETE.json" ]]; then
  CUDA_VISIBLE_DEVICES=0 "$PYTHON" -u "$CODE/train_nuscenes_tiny_point_vf7_router_ddp.py" \
    "${COMMON_ARGS[@]}" --out-dir "$SMOKE_OUT" --epochs 1 --batch-size 4 \
    --max-train-frames 8 >"$SMOKE_OUT/smoke.log" 2>&1
fi
grep -q 'bpp_monotonic=True' "$SMOKE_OUT/smoke.log"
touch "$STATE/SMOKE_COMPLETE"

record "Stage 2/6 seven-GPU TinyPoint-VF7 training"
if [[ ! -s "$TRAIN_OUT/TRAINING_COMPLETE.json" ]]; then
  CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6 "$PYTHON" -m torch.distributed.run \
    --master_addr=127.0.0.1 --master_port=29670 --nproc_per_node=7 \
    "$CODE/train_nuscenes_tiny_point_vf7_router_ddp.py" \
    "${COMMON_ARGS[@]}" --out-dir "$TRAIN_OUT" --epochs 20 --patience 5 --batch-size 8 \
    >"$TRAIN_OUT/train.log" 2>&1
fi
[[ -s "$TRAIN_OUT/checkpoints/epoch_best.pth" ]]
touch "$STATE/TRAIN_COMPLETE"

record "Stage 3/6 export official-val routing candidates"
mkdir -p "$TRAIN_OUT/candidate_predictions" "$TRAIN_OUT/map_checkpoint_selection"
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6 "$PYTHON" -m torch.distributed.run \
  --master_addr=127.0.0.1 --master_port=29671 --nproc_per_node=7 \
  "$CODE/export_nuscenes_tiny_point_vf7_candidates_multigpu.py" \
  --training-dir "$TRAIN_OUT" --output-dir "$TRAIN_OUT/candidate_predictions" \
  >"$TRAIN_OUT/export.log" 2>&1
touch "$STATE/EXPORT_COMPLETE"

record "Stage 4/6 evaluate the training-loss-selected checkpoint on official validation"
if [[ ! -s "$TRAIN_OUT/map_checkpoint_selection/MAP_SELECTION_COMPLETE.json" ]]; then
  "$PYTHON" -u "$CODE/select_nuscenes_router_by_map_bpp.py" \
    ...
    >"$TRAIN_OUT/map_selection.log" 2>&1
fi
[[ -s "$TRAIN_OUT/best_map_bpp.pth" ]]
touch "$STATE/SELECTION_COMPLETE"

record "Stage 5/6 draw total mAP-BPP test plot"
...
record "Stage 6/6 finalize"
printf '{"status":"complete","model_alias":"TinyPoint-VF7","task":"nuScenes CenterPoint detection","metric":"official 10-class mAP"}\n' >"$TRAIN_OUT/PIPELINE_COMPLETE.json"
touch "$STATE/COMPLETE"
record "TinyPoint-VF7 nuScenes pipeline complete"
