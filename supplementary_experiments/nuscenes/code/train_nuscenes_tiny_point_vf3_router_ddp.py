#!/usr/bin/env python3
"""Train TinyPoint-VF3 on all nuScenes training labels.
...
Checkpoint selection and early stopping use minimum full-training regression
loss.  Official nuScenes validation is used only for the final test curve.
"""
...
from absolute_loss_monotonic_rate_proxy import rd_levels
from tiny_point_vf3_absolute_loss_monotonic_rate_proxy import (
...
class NuScenesRouterDataset(Dataset):
    def __init__(
        ...
        split_files = (
            list(split_file)
            if isinstance(split_file, (list, tuple))
            else [split_file]
        )
        split_ids = []
        for current_split in split_files:
            split_ids.extend(read_split(current_split))
        split_ids = list(dict.fromkeys(split_ids))
        if int(limit) > 0:
        ...
        print(
            f"Dataset {[str(value) for value in split_files]}: "
            f"samples={len(self.items)} spatial_shape={self.spatial_shape} "
...
def build_model(variant, spatial_shape, feat_dim, loss_scales, mean_log_bpp):
    if variant != "tiny_point_vf3":
        raise ValueError(f"Unsupported model variant: {variant}")
    return TinyPointVF3AbsoluteLossMonotonicRateProxy(
        feat_dim=feat_dim,
...
@torch.no_grad()
def load_lite_heads_into_tiny(model, checkpoint_path):
    report = model.load_compatible_heads(Path(checkpoint_path))
    if report["loaded_head_tensor_count"] != 28:
        raise RuntimeError(f"Expected all 28 Lite-S3 head tensors, got {report}")
    return report


@torch.no_grad()
def load_full_tiny_into_tiny(model, checkpoint_path):
    return model.load_from_vf7_checkpoint(Path(checkpoint_path))
...
def pack_voxel_features(features, coords, batch_size: int, training: bool):
    ...
    if min(lengths) <= 0:
        raise RuntimeError(f"Empty TinyPoint-VF3 cloud in batch: {lengths}")
...
def forward_batch(model, batch, device, args, training):
    ...
    voxel_features = select_vf3_features(voxel_features)
    packed, valid_mask, lengths = pack_voxel_features(
        voxel_features, voxel_coords, len(points), training
    )
    if packed.shape[-1] != 3:
        raise RuntimeError(f"TinyPoint-VF3 expected 3 features, got {packed.shape[-1]}")
    output = model(packed, valid_mask)
    output["active_voxel_counts"] = lengths
    return output
    ...
    for batch_index, batch in enumerate(progress):
        ...
        if batch_index == 0 and rank == 0:
            print(
                ...
                f"active_voxels={sum(output['active_voxel_counts'])} "
                f"voxel_min={min(output['active_voxel_counts'])} "
                f"voxel_max={max(output['active_voxel_counts'])} "
...
def save_checkpoint(path, model, optimizer, scheduler, epoch, metrics, args, lambdas, init_report):
    torch.save({
        ...
        "model_alias": "TinyPoint-VF3",
        "model_type": "tiny_point_vf3_six_direct_absolute_loss_heads_plus_monotonic_six_bpp",
        "input_feature_dim": 3,
...
def parse_args():
    ...
    parser.add_argument("--val-loss-csv", default="")
    parser.add_argument("--train-bpp-csv", required=True)
    parser.add_argument("--val-bpp-csv", default="")
    parser.add_argument("--train-split", required=True, nargs="+")
    ...
    parser.add_argument("--model-variant", choices=["tiny_point_vf3"], default="tiny_point_vf3")
    parser.add_argument("--init-checkpoint", required=True)
    parser.add_argument(
        "--init-kind",
        choices=["tiny_point_full"],
        default="tiny_point_full",
    )
...
def main():
    ...
    init_report = (
        load_full_tiny_into_tiny(model, args.init_checkpoint)
    )
    if rank == 0:
        run_args = {
            **vars(args),
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if rank == 0:
            ...
            save_checkpoint(output / "latest.pth", bare, optimizer, scheduler, epoch, checkpoint_metrics, args, lambdas, init_report)
            if improved:
                save_checkpoint(output / "best.pth", bare, optimizer, scheduler, epoch, checkpoint_metrics, args, lambdas, init_report)
                save_checkpoint(output / "checkpoints" / "epoch_best.pth", bare, optimizer, scheduler, epoch, checkpoint_metrics, args, lambdas, init_report)
            print(
    ...
    if rank == 0:
        summary = {
            ...
            "model_alias": "TinyPoint-VF3",
            "model_variant": args.model_variant,
            "parameter_counts": count_parameters(model.module if isinstance(model, DistributedDataParallel) else model),
