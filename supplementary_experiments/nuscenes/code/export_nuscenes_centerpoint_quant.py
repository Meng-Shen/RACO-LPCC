...
The script is intentionally single-GPU.  A shell queue launches four
independent shards on GPUs 0--3, which avoids distributed result-ordering
issues and makes every shard safely resumable.
"""
...
def make_dataset_cfg(cfg, data_root: str, split: str, qstep_mm: float,
    ...
    pipeline = [
        dict(
            ...
            use_dim=[0, 1, 2], backend_args=None),
    ]
    if loss_mode:
...
def main():
    ...
    for qstep in qsteps:
        dataset = DATASETS.build(make_dataset_cfg(
            cfg, args.data_root, args.split, qstep, loss_mode))
        dataset.full_init()
        datasets.append(dataset)
        quantizers.append(find_quantizer(dataset))
    total = len(datasets[0])
    if any(len(dataset) != total for dataset in datasets):
        raise RuntimeError('Dataset lengths differ between levels')
    ...
    for ordinal, index in enumerate(indices, 1):
        sample_idx, lidar_path = sample_identity(datasets[0], index)
        if loss_mode:
            stats_by_level = []
            for dataset, quantizer in zip(datasets, quantizers):
    ...
    manifest = dict(
        ...
        geometry_only=True, single_keyframe=True,
