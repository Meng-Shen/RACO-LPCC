#!/usr/bin/env python3
"""Train TinyPoint-VF7 on all nuScenes training labels.
...
def build_model(variant, spatial_shape, feat_dim, loss_scales, mean_log_bpp):
    if variant != "tiny_point_vf7":
        raise ValueError(f"Unsupported model variant: {variant}")
    return TinyPointVF7AbsoluteLossMonotonicRateProxy(
        feat_dim=feat_dim,
...
def load_full_tiny_into_tiny(model, checkpoint_path):
    return model.load_full_checkpoint(Path(checkpoint_path))
...
def pack_voxel_features(features, coords, batch_size: int, training: bool):
    ...
    if min(lengths) <= 0:
        raise RuntimeError(f"Empty TinyPoint-VF7 cloud in batch: {lengths}")
...
def forward_batch(model, batch, device, args, training):
    ...
    if packed.shape[-1] != 7:
        raise RuntimeError(f"TinyPoint-VF7 expected 7 features, got {packed.shape[-1]}")
...
def save_checkpoint(path, model, optimizer, scheduler, epoch, metrics, args, lambdas, init_report):
    torch.save({
        ...
        "model_type": "tiny_point_vf7_six_direct_absolute_loss_heads_plus_monotonic_six_bpp",
        "input_feature_dim": 7,
        "input_feature_semantics": (
...
def parse_args():
    ...
    parser.add_argument("--model-variant", choices=["tiny_point_vf7"], default="tiny_point_vf7")
    parser.add_argument("--init-checkpoint", required=True)
    parser.add_argument(
        "--init-kind",
        choices=["lite_s3_heads", "tiny_point_full"],
        default="lite_s3_heads",
    )
...
def main():
    ...
    init_report = (
        ...
        else load_lite_heads_into_tiny(model, args.init_checkpoint)
    )
    if rank == 0:
        run_args = {
            **vars(args),
