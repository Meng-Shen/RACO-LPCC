#!/usr/bin/env python3
"""TinyPoint router for normalized small point clouds.
...
and analytical RD selection inputs are shape-compatible with Lite-S3.
"""
...
class TinyPointCostBackbone(nn.Module):
    ...
    def __init__(self, input_channels: int = 3, feat_dim: int = 256,
        ...
        # Intentionally identical to Lite-S3.
...
class TinyPointAbsoluteLossMonotonicRateProxy(nn.Module):
    """TinyPoint backbone plus unchanged direct-six-loss and monotonic-BPP heads."""

    def __init__(self, feat_dim: int, loss_scales: Iterable[float],
                 mean_log_bpp: Iterable[float], input_channels: int = 3) -> None:
        super().__init__()
        self.base = TinyPointCostBackbone(input_channels, feat_dim, NUM_LEVELS)
        # Intentionally identical to Lite-S3.
    ...
    def load_compatible_heads(self, checkpoint_path: Path) -> Dict[str, Any]:
        """Load exactly shape-compatible Lite-S3 loss/rate head tensors only."""
    ...
    def load_full_checkpoint(self, checkpoint_path: Path) -> Dict[str, Any]:
        """Load a TinyPoint checkpoint while keeping current-dataset scales."""
...
def count_parameters(model: TinyPointAbsoluteLossMonotonicRateProxy) -> Dict[str, int]:
    ...
    return {
        ...
        "tiny_point_backbone": int(backbone),
