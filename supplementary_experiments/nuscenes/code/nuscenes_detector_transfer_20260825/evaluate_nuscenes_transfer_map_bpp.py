...
def main() -> None:
    ...
    parser.add_argument("--detector-name", default="Transferred XYZ-only detector")
    parser.add_argument("--routing-name", default="CenterPoint routing transfer")
    ...
    for series, label, marker, color in (
        ...
        ("rate_aware_proxy", f"{args.routing_name} (epoch {args.checkpoint_epoch})", "^", "#E45756"),
    ):
        current = sorted(
    ...
    ax.set_title(f"nuScenes XYZ-only 10-sweep {args.detector_name}: mAP–BPP")
    ...
    summary = {
        ...
        "detector": f"{args.detector_name} XYZ-only: keyframe plus 9 historical sweeps",
