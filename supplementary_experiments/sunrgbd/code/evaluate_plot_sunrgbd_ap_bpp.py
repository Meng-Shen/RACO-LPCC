#!/usr/bin/env python3
"""Evaluate and plot only fixed G-PCC and latest Lite-S3 SUN RGB-D curves."""
...
def evaluate(records, levels, logger):
    ...
    return indoor_eval(
        ...
        SUNRGBDDataset.METAINFO['classes'], logger=logger, box_mode_3d=box_mode,
    )
...
def main():
    ...
    for level, qstep in enumerate(manifests[0]['qsteps_mm_coarse_to_fine']):
        ...
        rows.append({
            ...
            'qstep_mm': qstep, 'lambda': '', 'bpp': total_bits / total_points,
            'mAP_0.25': metrics['mAP_0.25'], 'mAP_0.50': metrics['mAP_0.50'],
    ...
    for rate_point in range(6):
        ...
        total_points = sum(gpcc[(sid, int(level))][1]
                           for sid, level in zip(scene_ids, levels))
        counts = np.bincount(levels, minlength=6)
        rows.append({
            'series': 'lite_s3_router', 'rate_point': rate_point,
            'qstep_mm': '', 'lambda': lambdas[rate_point],
            'bpp': total_bits / total_points,
            'mAP_0.25': metrics['mAP_0.25'], 'mAP_0.50': metrics['mAP_0.50'],
            'selection_counts': ';'.join(str(int(value)) for value in counts),
        })

    csv_path = args.output_dir / 'sunrgbd_lite_s3_vs_gpcc_ap_bpp.csv'
    ...
    for axis, key, title in (
        ...
        for series, label, color, marker in (
            ...
            ('lite_s3_router', 'Lite-S3 router', '#E45756', 's'),
        ):
            selected = sorted(
        ...
        axis.legend()
    fig.tight_layout()
    png_path = args.output_dir / 'sunrgbd_lite_s3_vs_gpcc_ap_bpp.png'
    fig.savefig(png_path, bbox_inches='tight')
    plt.close(fig)
    manifest = {
        'status': 'complete', 'dataset': 'SUN RGB-D val',
        'curves_shown': ['fixed G-PCC baseline', 'latest Lite-S3 router'],
