...
STATE="$ROOT/state"
LOGS="$ROOT/logs/final_qsteps_160_120_100_80_60_40"
EXTRA="$ROOT/supplemental/qstep_100"
BASE="$ROOT/revised_qsteps_160_120_80_60_40_20"
COMBINED="$ROOT/final_qsteps_160_120_100_80_60_40"
RESULTS="$ROOT/results/oracle_vs_gpcc_train_preview_qsteps_160_120_100_80_60_40"
STAGE=initializing

mkdir -p "$STATE" "$LOGS" "$EXTRA" "$COMBINED" "$RESULTS"
trap 'printf "{\"status\":\"failed\",\"stage\":\"%s\",\"time\":\"%s\"}\n" "$STAGE" "$(date -Is)" > "$STATE/FINAL_QSTEPS_TRAIN_PREVIEW_FAILED.json"' ERR
date -Is > "$STATE/FINAL_QSTEPS_TRAIN_PREVIEW_STARTED"
...
run_loss() {
  ...
  "$PY" "$CODE/merge_sunrgbd_loss_shards.py" \
    ...
    --expected-scenes 5285 --num-levels 1 \
    --output "$EXTRA/sunrgbd_train_absolute_losses_qstep_100.csv"
}
...
run_gpcc() {
  ...
  for shard in $(seq 0 13); do
    (
      ...
      "$PY" "$CODE/measure_sunrgbd_gpcc.py" \
        ...
        --split-file "$DATA/sunrgbd_trainval/train_data_idx.txt" \
        --split-name train --qsteps-mm 100 \
  ...
  "$PY" "$CODE/measure_sunrgbd_gpcc.py" \
    ...
    --split-file "$DATA/sunrgbd_trainval/train_data_idx.txt" \
    --expected-scenes 5285 --qsteps-mm 100 \
    --output "$EXTRA/sunrgbd_train_gpcc_qstep_100.csv"
}
...
wait "$loss_pid"
wait "$gpcc_pid"
date -Is > "$STATE/FINAL_QSTEPS_LOSS_GPCC_COMPLETE"
...
for pid in "${pids[@]}"; do wait "$pid"; done
date -Is > "$STATE/FINAL_QSTEPS_PREDICTIONS_COMPLETE"

STAGE=combining_cached_assets
PYTHONPATH="$MMDET:$CODE" "$PY" "$CODE/combine_sunrgbd_qstep_assets.py" \
  --old-loss "$BASE/sunrgbd_train_absolute_losses.csv" \
  --extra-loss "$EXTRA/sunrgbd_train_absolute_losses_qstep_100.csv" \
  --output-loss "$COMBINED/sunrgbd_train_absolute_losses.csv" \
  --old-gpcc "$BASE/sunrgbd_train_gpcc.csv" \
  --extra-gpcc "$EXTRA/sunrgbd_train_gpcc_qstep_100.csv" \
  --output-gpcc "$COMBINED/sunrgbd_train_gpcc.csv" \
  --old-predictions "$BASE/predictions" \
  --extra-predictions "$EXTRA/predictions" \
  --output-predictions "$COMBINED/predictions" \
  --base-qsteps 160 120 80 60 40 20 \
  --extra-qsteps 100 \
  --target-qsteps 160 120 100 80 60 40 \
...
STAGE=complete
date -Is > "$STATE/FINAL_QSTEPS_TRAIN_PREVIEW_COMPLETE"
rm -f "$STATE/FINAL_QSTEPS_TRAIN_PREVIEW_FAILED.json"
