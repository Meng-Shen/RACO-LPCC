...
with torch.no_grad():
    for file_path in tqdm(file_path_ls):
        ...
        ################################ Decompress
        base_x_coords = torch.tensor(
            base_x_coords.reshape(-1, 3),
            dtype=torch.int32,
            device=device
        )
        base_x_feats = torch.tensor(
            base_x_feats.reshape(-1, 1),
            dtype=torch.uint8,
            device=device
        )
        # print(base_x_coords[0, :]) #[1, 3, 3]
        ...
        posQ = torch.tensor(posQ, dtype=torch.float32, device=scan.device)
