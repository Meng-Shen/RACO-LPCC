...
class Tester():
    ...
    def test_bitrates(self, filedir, idx_file):
        ...
        # 10 种量化步长
        posQuantscale_list = [1/8, 1/16, 1/64, 1/128]
...
if __name__ == '__main__':
    ...
    if os.path.isdir(args.testdata):
        ...
        print(f"Reading validation list from: {val_txt_path}")
        
        # 逐行读取 val.txt
        with open(val_txt_path, 'r') as f:
            val_ids = [line.strip() for i, line in enumerate(f) if i % 10 == 0 and line.strip()]
            
        # 根据 val_ids 构建完整的文件路径列表
        filedir_list = []
        for vid in val_ids:
            ...
            if os.path.exists(bin_file):
                ...
                print(f"Warning: 验证集文件缺失，找不到 {bin_file}")
    else:
        # 传入为单个 .bin 文件
