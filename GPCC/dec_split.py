...
from extention.gpcc_geo import gpcc_encode, gpcc_decode  # 这里增加 gpcc_decode
...
class Tester():
    ...
    def test_bitrates(self, filedir, idx_file):
        ...
        scale_combinations = [
            (1/256, 1/2048), (2/256, 1/2048),(3/256, 1/2048),(1/64, 1/2048), (1/64, 1/512),
        ...
        for combo_idx, (posQ_fg, posQ_bg) in enumerate(scale_combinations):
            ...
            if len(coords_fg) > 0:
                ...
                if os.path.exists(tmp_fg_bin):
                    ...
                    enc_time_total += log_fg.get('Processing time (wall)', 0)

                    # ===================== 解码 =====================
                    tmp_fg_dec_ply = os.path.join(self.args.tmp_dir, f"{filename}_tmp_fg_dec_{combo_idx}.ply")
                    with suppress_stderr():
                        log_fg_dec = gpcc_decode(tmp_fg_bin, tmp_fg_dec_ply)
                    dec_time_total += log_fg_dec.get('Processing time (wall)', 0)
                    if os.path.exists(tmp_fg_dec_ply):
                        os.remove(tmp_fg_dec_ply)
                    # =================================================
            ...
            if len(coords_bg) > 0:
                ...
                if os.path.exists(tmp_bg_bin):
                    ...
                    enc_time_total += log_bg.get('Processing time (wall)', 0)

                    # ===================== 解码 =====================
                    tmp_bg_dec_ply = os.path.join(self.args.tmp_dir, f"{filename}_tmp_bg_dec_{combo_idx}.ply")
                    with suppress_stderr():
                        log_bg_dec = gpcc_decode(tmp_bg_bin, tmp_bg_dec_ply)
                    dec_time_total += log_bg_dec.get('Processing time (wall)', 0)
                    if os.path.exists(tmp_bg_dec_ply):
                        os.remove(tmp_bg_dec_ply)
                    # =================================================
            ...
            results = {
                ...
                'dec_time': round(dec_time_total, 4)  # 新增解码时间输出
            }
