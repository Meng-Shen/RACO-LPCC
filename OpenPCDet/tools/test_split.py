...
def parse_config():
    ...
    parser.add_argument('--quant_map', type=str, default=DEFAULT_QUANT_MAP_STR,
                        help='Quantization combinations. Format: fg,bg;fg,bg. Example: 1/256,0;2/256,0;1/64,1.5/512')
...
def main():
    ...
    logger.info(f'Using {len(quant_map)} split quantization combinations from --quant_map')
    for combo_idx, (scale_fg, scale_bg) in enumerate(quant_map):
        logger.info(f'  combo_{combo_idx}: fg={scale_fg:.6f}, bg={scale_bg:.6f}')
    ...
    logger.info(f'********************** All {len(quant_map)} Split Combinations Evaluated Successfully **********************')
