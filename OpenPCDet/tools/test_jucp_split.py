...
def parse_config():
    ...
    parser.add_argument('--quant_map', type=str, default=DEFAULT_QUANT_MAP_STR,
                        help='Quantization combinations. Format: fg,bg;fg,bg. Must match test_split.py/new_split.py')
...
def main():
    ...
    # =========================================================================
    # 动态注入 JUCP Split 双量化逻辑 (Monkey Patch Dataset)
    # =========================================================================
    ...
    logger.info('Using JUCP label -> quantization mapping:')
    for label in sorted(label_to_quant):
        scale_fg, scale_bg = label_to_quant[label]
        logger.info(f'  Label {label}: fg={scale_fg:.6f}, bg={scale_bg:.6f}')

    jucp_map = {}
    for _, row in jucp_df.iterrows():
        fid = str(row['frame_id']).zfill(6)
        label = int(row['jucp_label'])
        if label not in label_to_quant:
            raise ValueError(f"Invalid jucp_label={label} for frame_id={fid}. Valid range: 0~{num_levels - 1}")
        jucp_map[fid] = label_to_quant[label]
    ...
    def patched_get_lidar(self, idx):
        ...
        # 默认如果没找到这一帧，则使用最大码率 (QUANT_MAP[0])
        scale_fg, scale_bg = self.jucp_map.get(str_idx, label_to_quant[0])
